"""Creates a desktop shortcut that can run Mars from anywhere."""
...
def run_as_admin():
    if args.depth < MAX_DEPTH:
        ...
        print(' ~  Finished setting up Mars')
    exit(0)


def create_desktop_shortcut():
    """Creates and saves a desktop shortcut using absolute paths"""
    print('\n[~] Creating desktop shortcut for Mars:')
    cwd = os.getcwd()
    ...
    shortcut_path = os.path.join(shell.SpecialFolders('Desktop'), 'Mars.lnk')
    ...
    print(' ~  Successfully created Mars shortcut')
