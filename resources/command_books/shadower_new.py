...
def step(direction, target):
    ...
    if direction == "up":
        ...
        Walk(target_x=target[0], max_steps=400).execute()
...
class JumpUp(Command):
    ...
    def main(self):
        ...
        if len(routine.minimap_data) > 0:
            hasRope = False
            cur_x = config.player_direction[0]
            cur_y = config.player_direction[1]
            for y in range(max(0, cur_y - 10), cur_y):
                if routine.minimap_data[cur_x][y] == 2:
                    hasRope = True
                    break
